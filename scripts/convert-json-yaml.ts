...
function getFormat(path: string): Format {
  ...
  throw new Error(
    `Unsupported file extension for ${path}. Use .json, .yaml, or .yml.`
  );
}
...
async function main() {
  ...
  if (inputFormat === outputFormat) {
    throw new Error(
      "Input and output formats are the same. Use one JSON file and one YAML file."
    );
  }
  ...
  console.log(
    `Converted ${inputFormat.toUpperCase()} to ${outputFormat.toUpperCase()}.`
  );
