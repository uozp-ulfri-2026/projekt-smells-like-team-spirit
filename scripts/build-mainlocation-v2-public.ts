...
function resolveOutputCountry(
  ...
  if (outputCountries.has(sourceCountry)) {
...
function buildGeoJson(
  ...
  for (const row of sourceRows) {
    ...
    const sourceCity = asOptionalString(
      row.location_model?.city ?? row.llm?.city
    );
    ...
    const coordinates = asCoordinatePair(
      coordinateFeature.geometry.coordinates
    );
