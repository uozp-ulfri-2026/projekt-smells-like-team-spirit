...
function offsetTopicCoordinates(
  ...
  const lngScale =
    KM_PER_LONGITUDE_DEGREE * Math.max(0.2, Math.cos(latitudeRadians));
...
export function CountryDots({
  ...
  const [labelAnchorsByKey, setLabelAnchorsByKey] = useState<
    ...
    for (const feature of data.features) {
      ...
      const topicGroups = Array.from(groupedIdsByTopic.entries()).sort(
        ([leftTopic, leftIds], [rightTopic, rightIds]) =>
          rightIds.length - leftIds.length ||
          leftTopic.localeCompare(rightTopic, "sl")
      );
        ...
        features.push({
          ...
          properties: {
            ...
            isSelected: selectedArticleId
              ? topicIds.includes(selectedArticleId)
              : false,
          },
        });
      });
  ...
  const refreshLabelAnchors = useCallback(() => {
    ...
    for (const candidate of candidates) {
      ...
      const matchingAnchorKeys = names.flatMap(
        (name) => targetKeysByCityName.get(name) ?? []
      );
      ...
      for (const anchorKey of matchingAnchorKeys) {
        ...
        if (distance > MAX_LABEL_ANCHOR_DISTANCE_KM) {
          continue;
        }
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MapEventWithFeatures) => {
      try {
        ...
        if (
          typeof primaryArticleId === "string" &&
          ids.includes(primaryArticleId)
        ) {
          ids.sort((left, right) => {
            if (left === primaryArticleId) {
              return -1;
            }
            if (right === primaryArticleId) {
              return 1;
            }
            return 0;
          });
  ...
  useEffect(() => {
    if (
      !(isLoaded && map?.getLayer(layer_id) && map.getLayer(pulse_layer_id))
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const setHover = (nextId: string | number | null) => {
      if (nextId === hovered_id_ref.current) {
        return;
      }
  ...
  useEffect(() => {
    if (!(isLoaded && map?.getLayer(pulse_layer_id) && selectedArticleId)) {
      return;
    }
